...
const names = ["Alice", "Bob", "Charlie", "David", "Eve", "Jim", "Jane", "Jeremiah", "zach"];
const hobbiesList = ["Reading", "Hiking", "Cooking", "Gaming", "Traveling", "Backpacking", "Canoeing"];
...
function generateRandomPerson(): Person {
  const name = getRandomElement(names); const age = Math.floor(Math.random() * 40) + 18;
  const numHobbies = Math.floor(Math.random() * 4) + 1;
  const hobbies: string[] = [];

  for (let i = 0; i < numHobbies; i++) {
    const hobby = getRandomElement(hobbiesList);hobbies.push(hobby);
  }
